            ...
            <div className="relative z-10 max-w-5xl mx-auto px-4">
                <div className="flex items-center justify-between mb-8">
                    <Link to="/profile" className="flex items-center gap-2 text-slate-400 hover:text-slate-900 transition-colors group">
                        <div className="w-10 h-10 rounded-full border-2 border-slate-200 flex items-center justify-center group-hover:bg-white group-hover:shadow-sm">
                            <ChevronLeft size={20} />
                        </div>
                            ...
                            <div className="divide-y divide-slate-100">
                                {order.orderItems.map((item, index) => (
                                    <div key={index} className="py-6 flex flex-col sm:flex-row gap-6 items-center group hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-blue-50/50 px-4 rounded-xl transition-all">
                                        <div className="w-24 h-24 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-2xl p-2 flex items-center justify-center shrink-0 group-hover:shadow-lg transition-all duration-500">
                                            <img 
                                                src={item.image} 
                        ...
                        <div className="bg-gradient-to-br from-white to-blue-50/30 rounded-3xl p-4 sm:p-6 md:p-8 lg:p-10 shadow-lg shadow-blue-100/30 border-2 border-slate-100 backdrop-blur-sm space-y-8">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between py-3 px-4 bg-slate-50 rounded-xl border border-slate-100">
                                    ...
                                    <span className="text-blue-600 font-black">${(order.itemsPrice || 0).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between py-3 px-4 bg-slate-50 rounded-xl border border-slate-100">
                                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Fiscal Override (Tax)</span>
                                    <span className="text-blue-600 font-black">${(order.taxPrice || 0).toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between py-3 px-4 bg-slate-50 rounded-xl border border-slate-100">
                                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">In-Transit Logistics</span>
                                    <span className="text-blue-600 font-black">${(order.shippingPrice || 0).toFixed(2)}</span>
                                </div>
                                <div className="pt-6 border-t-2 border-blue-100">
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        <span className="text-3xl sm:text-4xl lg:text-5xl font-black text-blue-600 tracking-tighter">${(order.totalPrice || 0).toFixed(2)}</span>
                                    </div>
                                </div>
                            </div>
