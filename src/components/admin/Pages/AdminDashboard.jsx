    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {loading ? (
                    <div className="col-span-full py-10 text-center text-slate-400">Loading metrics...</div>
                ) : error ? (
                                ...
                                {loading ? (
                                    ...
                                    analytics.recentOrders.map((order) => (
                                        <tr key={order._id} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => navigate('/admin/orders')}>
                                            ...
                                            <td className="px-6 py-4 text-right text-slate-500">{new Date(order.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</td>
                                        </tr>
                                    ))
                                ) : (
